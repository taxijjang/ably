...
class SMSVerifyTestCase(APITestCase):
    ...
    def test_회원가입_문자_메시지_인증_성공(self, phone_number="01063110710"):
        회원가입전정보 = {
        ...
        verify_data = {
            "phone_number": 회원가입전정보.get("phone_number"),
            "type": 회원가입전정보.get("type"),
            "code": auth_sms.code,
        }
    ...
    def test_비밀번호_재설정_문자_메시지_인증_성공(self, phone_number="01063110710"):
        비밀번호재설정정보 = {
            "phone_number": phone_number,
        ...
        verify_data = {
            "phone_number": 비밀번호재설정정보.get("phone_number"),
            "type": 비밀번호재설정정보.get("type"),
            "code": auth_sms.code,
        }
        response = client.post(reverse("api:sms:sms_verify"), verify_data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
